...
summary(mod_max_score_status_sex)
...
mod1_breeding <- 
  lme4::glmer(prop_molt_score ~ 
                 date_J * breeding_date_J +
         (1 | rings_comb),
       data = ind_prop_molt_scores, 
       family = binomial)

summary(mod1_breeding)

plot(allEffects(mod1_breeding), type = "response")




# strong date effect, but no status effect
tbl_regression(mod1_breeding, intercept = TRUE, 
               label = list(date_J ~ "Date", breeding_date_J ~ "Breeding timing"))
...
pr <- ggeffects::predict_response(mod1_breeding, c("date_J[30:293]", "breeding_date_J [80,100,120,140,160,180,200,220]"))
...
mod1_fits <- 
  as.data.frame(pr) %>% 
  rename(date_J = x,
         breeding_date_J = group) %>% 
  left_join(., dates_for_plot, by = "date_J")

# plot the model
ggplot() +
  geom_line(data = mod1_fits, 
            aes(x = date, y = predicted, color = breeding_date_J)) +
  geom_ribbon(data = mod1_fits, 
              aes(x = date, ymax = conf.high, ymin = conf.low, fill = breeding_date_J),
              lwd = 1, alpha = 0.25) +
  luke_theme +
  theme(legend.position = c(0.3, 0.2),
        legend.justification = c(1, 0),
...
########### How the timing of breeding change with aging

# when males age they stay breeding later in the season, but the opposite in females
mod2_breeding <- 
...
summary(mod2_breeding)

plot(allEffects(mod2_breeding), type = "response")

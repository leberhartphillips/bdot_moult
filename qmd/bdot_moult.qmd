...
dat %>% 
  dplyr::select(rings_comb, migratory_status) %>% 
  # filter(rings_comb == "RWLB") %>% 
  # mutate(month = month(date)) %>% 
  filter(rings_comb %in% c("RBOG", "RBGB", "RROW", "RBWO", "RRGO", "RBWR", "RLYL", "RWBG", "RLYR", "RRBR", "RBBY", "RWBY", "RRLR", "RRYY", "RWLB")) %>%
  # filter(month(date) < 9 & month(date) > 2) %>%
  # dplyr::select(rings_comb, season, month, date) %>% 
  distinct() %>% 
  filter(migratory_status == "R")

dat %>% 
  dplyr::select(rings_comb, sourcefile, date) %>% 
  # filter(rings_comb == "RWLB") %>% 
  # mutate(month = month(date)) %>% 
  # filter(rings_comb %in% c("RBOG", "RBGB", "RROW", "RBWO", "RRGO", "RBWR", "RLYL", "RWBG", "RLYR", "RRBR", "RBBY", "RWBY", "RRLR", "RRYY", "RWLB")) %>%
  filter(rings_comb == "RBRL") %>% 
  # filter(month(date) < 9 & month(date) > 2) %>%
  # dplyr::select(rings_comb, season, month, date) %>% 
  distinct() %>% print(n = nrow(.))

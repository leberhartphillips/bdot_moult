...
# summarize the number of individuals in the dataset...94 unique combos
dat %>%
...
stats_mod_max_score_age_sex <- readRDS(file = "/Users/luketheduke2/ownCloud/kemp_projects/bdot/moult/out/stats_mod_max_score_age_sex.rds")
```
...
pr <- ggeffects::predict_response(mod_max_score_age_sex)

# join the back-transformed dates to model fits
mod_max_score_age_sex_fits <- 
  as.data.frame(pr)

ggplot() +
  geom_ribbon(data = mod_max_score_age_sex_fits, 
                aes(x = age.x, ymin = age.conf.low, ymax = age.conf.high), fill = "steelblue", alpha = 0.5) +
  geom_line(data = mod_max_score_age_sex_fits, 
             aes(x = age.x, y = age.predicted), color = "steelblue") +
  geom_jitter(data = ind_breeding_scores_age,
             aes(x = age, 
                 y = max_breeding_score), 
             width = 0.05, height = 0.1, shape = 21, size = 3, alpha = 0.75, fill = "steelblue", alpha = 1) +
  theme(legend.position = "none",
        text = element_text(family = "lato"),
        axis.ticks = element_blank(),
        axis.text = element_text(size = 10, colour = "grey40"),
...
stats_mod_max_score_status_sex <- readRDS(file = "/Users/luketheduke2/ownCloud/kemp_projects/bdot/moult/out/stats_mod_max_score_status_sex.rds")
```
...
effects_mod_max_score_status_sex2 <- 
  as.data.frame(allEffects(mod_max_score_status_sex)$migratory_status) %>%
...
ggplot() +
  geom_errorbar(data = effects_mod_max_score_status_sex2, 
                aes(x = x_mean, ymin = lower, ymax = upper), width = 0.05) +
  geom_point(data = effects_mod_max_score_status_sex2, 
  ...
  geom_jitter(data = ind_breeding_scores_status,
             aes(x = as.numeric(factor(migratory_status)), 
                 y = max_breeding_score, fill = migratory_status), 
             width = 0.05, height = 0.1, shape = 21, size = 3, alpha = 0.75) +
  theme(legend.position = "none",
        text = element_text(family = "lato"),
        axis.ticks = element_blank(),

...
dat <- 
  read.csv("/Users/luketheduke2/ownCloud/kemp_projects/bdot/moult/data/Dataset_Molt_Final - modified 7.11.2024.csv", colClasses = "character") %>% 
...
dat_sexes <- 
  read.csv("/Users/luketheduke2/ownCloud/kemp_projects/bdot/moult/data/sex_status_banding.csv", colClasses = "character") %>% 
  mutate(Rings_comb = str_sub(Rings_comb, 1, 4)) %>% 
  rename_with(tolower) %>% # convert all column headers to lowercase
  distinct()
...
# NOTE FOR BASHAR (23-01-2025): 
`would be great to add a figure here showing example photos of an individual male and female over several phases of the moult. For example, two-columns with male on left, female on right, and 5 rows of images starting with their fullest and most immaculate state and ending with their basic plumage. Would be useful to show the scores you gave for these individuals in the figure`
...
stats_mod_max_score <- readRDS(file = "/Users/luketheduke2/ownCloud/kemp_projects/bdot/moult/out/stats_mod_max_score.rds")
```
...
stats_mod_max_score_age <- readRDS(file = "/Users/luketheduke2/ownCloud/kemp_projects/bdot/moult/out/stats_mod_max_score_age.rds")
```
...
stats_mod_max_score_status <- readRDS(file = "/Users/luketheduke2/ownCloud/kemp_projects/bdot/moult/out/stats_mod_max_score_status.rds")
```
...
```{r}
mod_moult_sex_bird_season_RIS <- 
  lme4::glmer(prop_molt_score_mode ~ 
                date_J + sex + 
         (date_J | rings_comb) + (date_J | season),
       data = ind_prop_molt_scores, 
       family = binomial)
```

```{r, echo=FALSE}
tbl_regression(mod_moult_sex_bird_season_RIS, intercept = TRUE, 
               label = list(date_J ~ "Date", sex ~ "Sex"))
```

```{r, echo=FALSE}
RIS_bird_season <- 
  augment(mod_moult_sex_bird_season_RIS) %>% 
  dplyr::select(prop_molt_score_mode, date_J, rings_comb, season, .fitted, sex)
...
df_bird_season <- 
  RIS_bird_season %>% 
  dplyr::group_by(sex, season, rings_comb, date_J) %>% 
  dplyr::summarise(RIS = mean(.fitted)) %>% 
  mutate(RIS_trans = exp(RIS)/(1 + exp(RIS))) %>% 
  left_join(., dates_for_plot, by = "date_J") %>% 
  mutate(rings_comb = as.factor(rings_comb),
         season = as.factor(season)) %>% 
  filter(date > as.Date("2021-11-01")) %>% 
  mutate(bird_season = paste(rings_comb, season, sep = "_"))

# extract predicted trends
pr <- ggeffects::predict_response(mod_moult_sex_bird_season_RIS, c("date_J [30:293]", "sex"))

# join the back-transformed dates to model fits
mod_moult_sex_bird_season_RIS_fits <- 
  as.data.frame(pr) %>% 
  rename(date_J = x,
         sex = group) %>% 
  left_join(., dates_for_plot, by = "date_J") %>% 
...
plot_rs_bird_season <-
  ...
  scale_x_date(date_labels = "%B", 
               expand = c(0.01, 0.01), 
               date_breaks = "1 month") +
  ggtitle("bird- and year-specific slopes of\npre-basic moult timing (random)")

# plot the model
plot_rs_bird_season_fs_sex <- 
  ggplot() +
  geom_line(data = mod_moult_sex_bird_season_RIS_fits, 
            aes(x = date, y = predicted, color = sex)) +
  geom_ribbon(data = mod_moult_sex_bird_season_RIS_fits, 
              aes(x = date, ymax = conf.high, ymin = conf.low, fill = sex),
              lwd = 1, alpha = 0.25) +
  geom_jitter(data = ind_prop_molt_scores %>% 
                left_join(., dates_for_plot, by = "date_J"),
              aes(x = date, y = prop_molt_score_mode, color = sex), 
  ...
  scale_fill_brewer(palette = "Dark2", direction = -1,
                    name = "Sex",
                    labels = c("Female (N = 49)", "Male (N = 32)")) +
  ggtitle("sex-specific slopes\nof pre-basic moult timing (fixed)")

plot_rs_bird_season + plot_rs_bird_season_fs_sex 
```
...
mode_scores <- ind_prop_molt_scores %>%
  group_by(rings_comb, season) %>%
  summarise(mode_score = max(prop_molt_score_mode, na.rm = TRUE), .groups = "drop")

# Step 2: Merge the max_scores back into the original data
mid_point_dates <-
  ind_prop_molt_scores %>%
  left_join(mode_scores, by = c("rings_comb", "season")) %>%  # Add the max_score
  group_by(rings_comb, season) %>%
  arrange(date_J) %>%  # Ensure data is sorted by date_J

  # Step 3: Find the latest date_J for the maximum prop_molt_score
  filter(prop_molt_score_mode == mode_score) %>%
  slice_max(order_by = date_J, n = 1, with_ties = FALSE) %>%
  rename(latest_mode_date_J = date_J) %>%

  # Step 4: Find the earliest date_J where prop_molt_score < max_score
  right_join(
    ind_prop_molt_scores %>%
      left_join(mode_scores, by = c("rings_comb", "season")) %>%  # Add the max_score here too
      group_by(rings_comb, season) %>%
      arrange(date_J) %>%
      filter(prop_molt_score_mode < mode_score) %>%
  ...
  mutate(mid_point_date_J = round((latest_mode_date_J + earliest_decline_date_J) / 2)) %>%

  # Select relevant columns
  select(rings_comb, season, latest_mode_date_J, earliest_decline_date_J, mid_point_date_J) %>%
  distinct()
...
RIS_bird_status <- 
  ...
  dplyr::select(prop_molt_score_mode, date_J, rings_comb, .fitted, migratory_status)
...
df_bird_status <- 
  RIS_bird_status %>% 
  dplyr::group_by(migratory_status, rings_comb, date_J) %>% 
  dplyr::summarise(RIS = mean(.fitted)) %>% 
  mutate(RIS_trans = exp(RIS)/(1 + exp(RIS))) %>% 
  left_join(., dates_for_plot, by = "date_J") %>% 
  mutate(rings_comb = as.factor(rings_comb)) %>% 
...
plot_rs_bird_status <-
  ...
  scale_x_date(date_labels = "%B", 
               expand = c(0.01, 0.01), 
               date_breaks = "1 month") +
  ggtitle("bird-specific slopes of\npre-basic moult timing (random)")
...
plot_rs_bird_status_fs <- 
  ...
  geom_jitter(data = ind_prop_molt_scores_status %>%
                left_join(., dates_for_plot, by = "date_J"),
              aes(x = date, y = prop_molt_score_mode, color = migratory_status),
              alpha = 0.25, shape = 20, width = 0, height = 0.025) +
  theme(legend.position = c(0.55, 0.1),
        # legend.background = element_rect(fill = 'transparent', color = NA),
        legend.justification = c(1, 0),
...
RIS_bird_age <- 
  ...
  dplyr::select(prop_molt_score_mode, date_J, rings_comb, .fitted, age)
...
df_bird_age <- 
  RIS_bird_age %>% 
  dplyr::group_by(age, rings_comb, date_J) %>% 
  dplyr::summarise(RIS = mean(.fitted)) %>% 
  mutate(RIS_trans = exp(RIS)/(1 + exp(RIS))) %>% 
  left_join(., dates_for_plot, by = "date_J") %>% 
  mutate(rings_comb = as.factor(rings_comb)) %>% 
...
plot_rs_bird_age_fs <- 
  ...
  geom_jitter(data = ind_prop_molt_scores_age %>%
              ...
              aes(x = date, y = prop_molt_score_mode, color = age),
...
breeding_data_2021 <-
  ...
  select(parent, first_date_breeding, last_date_breeding, nest_id, OBJECTID)
...
breeding_2021_final <- left_join(breeding_data_2021, breeding_data_2021_, by = "nest_id") %>%
  ...
  mutate(first_date_breeding_ = min(found_date, hatch_date, fail_date, na.rm = TRUE)) %>% 
  ...
  select(parent, first_date_breeding_final, last_date_breeding_final) %>% 
  # specify the season as the first calender year
...
ind_prop_molt_scores_last_breeding_date <-
  ...
  dplyr::select(rings_comb, season, sex, last_breeding_date_J, last_breeding_date_J_snum, prop_molt_score_mode, mode_breeding_score, age, migratory_status) %>% 
  distinct() %>%
  group_by(sex) %>%
  mutate(std_mode_breeding_score = (mode_breeding_score - mean(mode_breeding_score, na.rm = TRUE)) / sd(mode_breeding_score, na.rm = TRUE)) %>%
  ungroup() %>% 
...
ind_prop_molt_scores_first_breeding_date <-
  ...
  arrange(rings_comb, season, date_diff) %>% 
  group_by(rings_comb, season) %>% slice(1) %>% 
  dplyr::select(rings_comb, season, sex, first_breeding_date_J, first_breeding_date_J_snum, prop_molt_score_mode, mode_breeding_score, age, migratory_status) %>% 
  distinct() %>%
  group_by(sex) %>%
  mutate(std_mode_breeding_score = (mode_breeding_score - mean(mode_breeding_score, na.rm = TRUE)) / sd(mode_breeding_score, na.rm = TRUE)) %>%
  ungroup() %>% 
...
```{r}
mod_mode_score_last_breeding_date <- 
  lme4::lmer(last_breeding_date_J_snum ~ 
                 std_mode_breeding_score +
         (1 | rings_comb) + (1 | season),
       data = ind_prop_molt_scores_last_breeding_date)

tbl_regression(mod_mode_score_last_breeding_date, intercept = TRUE)
plot(allEffects(mod_mode_score_last_breeding_date), type = "response")
```
...
```{r}
mod_mode_score_first_breeding_date <- 
  lme4::lmer(first_breeding_date_J_snum ~ 
                 std_mode_breeding_score +
         (1 | rings_comb) + (1 | season),
       data = ind_prop_molt_scores_first_breeding_date)

tbl_regression(mod_mode_score_first_breeding_date, intercept = TRUE)
plot(allEffects(mod_mode_score_first_breeding_date), type = "response")
```
...
```{r}
mod_last_breeding_date_mode_score <- 
  lme4::lmer(std_mode_breeding_score ~ 
                 last_breeding_date_J_snum +
         (1 | rings_comb) + (1 | season),
       data = ind_prop_molt_scores_last_breeding_date)

tbl_regression(mod_last_breeding_date_mode_score, intercept = TRUE)
plot(allEffects(mod_last_breeding_date_mode_score), type = "response")
```
...
plot(allEffects(mod_last_breeding_date_prop_molt_mid), type = "response")
```

#### effect of prolonged breeding on prolonged retention of full plumage
do individuals with late breeding attempts retain their full breeding plumage longer? No effect
```{r}
mod_last_breeding_date_prop_molt_latest <- 
  lmer(latest_mode_date_J ~ last_breeding_date_J_snum + 
         (1 | rings_comb) + (1 | season),
       data = ind_prop_molt_scores_last_breeding_date)

tbl_regression(mod_last_breeding_date_prop_molt_latest, intercept = TRUE)
plot(allEffects(mod_last_breeding_date_prop_molt_latest), type = "response")
```
...
plot(allEffects(mod_age_last_breeding_date), type = "response")
...
plot(allEffects(mod_age_first_breeding_date), type = "response")

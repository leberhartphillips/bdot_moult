...
assess how many individuals have more than a single seasons of data
```{r, echo = FALSE}
dat %>%
...
wrangle data by calculating the individual proportional moult scores by comparing each score to a given individual's max (determined in previous chunk). Piece apart sex-specific variation in the timing of pre-basic moult while accounting for sex differences in maximum rufous band score
```{r}
ind_prop_molt_scores <- 
  ...
  select(rings_comb, season, date_J, sex, score, max_breeding_score, std_max_breeding_score, prop_molt_score) %>% 
...
mixed effects binomial model assessing an interaction between sex and date on changes in moult scores while controlling for between sex differences in the maximum breeding score
```{r, warning=FALSE}
mod_moult_sex <- 
...
RIS_season <- augment(mod_moult_sex_season_RIS) %>% dplyr::select(prop_molt_score, date_J, season, .fitted, sex)
...
df_season <- 
  RIS_season %>% 
  dplyr::group_by(sex, season, date_J) %>% 
...
# extract predicted trends
pr <- ggeffects::predict_response(mod_moult_sex_season_RIS, c("date_J [30:293]", "sex"))

# join the back-transformed dates to model fits
mod_moult_sex_season_RIS_fits <- 
  as.data.frame(pr) %>% 
  rename(date_J = x,
         sex = group) %>% 
  left_join(., dates_for_plot, by = "date_J") %>% 
...
plot_rs_season <-
  ggplot(data = df_season, 
         aes(x = date, y = RIS_trans, colour = season, group = season)) + 
  geom_line() +
  ...
  scale_colour_brewer(palette = "Set2",
                      name = "Season") +
  ggtitle("year-specific slopes of\npre-basic moult timing (random)")

# plot the model
plot_rs_season_fs_sex <- 
  ggplot() +
  geom_line(data = mod_moult_sex_season_RIS_fits, 
            aes(x = date, y = predicted, color = sex)) +
  geom_ribbon(data = mod_moult_sex_season_RIS_fits, 
...
plot_rs_season + plot_rs_season_fs_sex 
...
```{r}
RIS_bird <- augment(mod_moult_sex_bird_RIS) %>% dplyr::select(prop_molt_score, date_J, rings_comb, .fitted, sex)

df_bird <- 
  RIS_bird %>% 
  dplyr::group_by(sex, rings_comb, date_J) %>% 
  dplyr::summarise(RIS = mean(.fitted))%>% 
  mutate(RIS_trans = exp(RIS)/(1 + exp(RIS))) %>% 
  left_join(., dates_for_plot, by = "date_J") %>% 
...
plot_rs_bird + plot_rs_bird_fs_sex 
...
RIS_bird_season <- 
  ...
  dplyr::select(prop_molt_score, date_J, rings_comb, season, .fitted, sex)
...
# Derive confidence intervals of effect sizes from parametric bootstrapping
tidy_mod_moult_sex_bird_season_RIS <-
  tidy(mod_moult_sex_bird_season_RIS, conf.int = TRUE)

# run rptR to obtain repeatabilities of random effects
rpt_mod_moult_sex <-
  rpt(prop_molt_score ~ date_J + sex + 
         (date_J | rings_comb) + (date_J | season),
      grname = c("rings_comb", "season", "Fixed"),
      data = ind_prop_molt_scores,
      datatype = "Binary",
...
# run partR2 on each model to obtain marginal R2, parameter estimates, and beta
# weights
R2m_mod_moult_sex_bird_season_RIS <-
  partR2(mod_moult_sex_bird_season_RIS,
         partvars = c("date_J", "sex"),
         R2_type = "marginal",
         nboot = 1000,
         CI = 0.95,
         max_level = 1)

R2c_mod_moult_sex_bird_season_RIS <-
  partR2(mod_moult_sex_bird_season_RIS,
         partvars = c("date_J", "sex"),
         R2_type = "conditional",
         nboot = 1000,
...
stats_mod_moult_sex <-
  list(mod = mod_moult_sex,
       tidy = tidy_mod_moult_sex,
       rptR = rpt_mod_moult_sex,
       partR2m = R2m_mod_moult_sex,
       partR2c = R2c_mod_moult_sex,
       data = ind_prop_molt_scores)
```

```{r, include=FALSE}
# saveRDS(stats_mod_moult_sex, file = "out/stats_mod_moult_sex.rds")
stats_mod_moult_sex <- readRDS(file = "/Users/luketheduke2/ownCloud/kemp_projects/bdot/moult/out/stats_mod_moult_sex.rds")
```
...
# Fixed effect sizes (standardized)
fixef_bw_Table <- 
  stats_mod_moult_sex$partR2m$BW %>% 
  as.data.frame() %>% 
  mutate(stat = "fixed_bw") %>% 
  rename(conf.low = CI_lower,
         conf.high = CI_upper)
...
# Semi-partial R2 estimates
R2Table <- 
  bind_rows(stats_mod_moult_sex$partR2m$R2,
            stats_mod_moult_sex$partR2c$R2[1,]) %>% 
  dplyr::select(term, estimate, CI_lower, CI_upper) %>% 
  as.data.frame() %>% 
  mutate(stat = "partR2") %>% 
  rename(conf.low = CI_lower,
...
# Adjusted repeatabilities
coefRptTable <- 
  stats_mod_moult_sex$rptR$R_boot %>% 
  dplyr::select(-Fixed) %>% 
  mutate(residual = 1 - rowSums(.)) %>% 
  apply(., 2, 
        function(x) c(mean (x), quantile (x, prob = c(0.025, 0.975)))) %>% 
...
mod_moult_sex_table <- 
  ...
  gt(rowname_col = "row",
     groupname_col = "effect") %>% 
  cols_label(comp_name = html("<i>Banded Dotterel pre-basic moult</i>"),
             estimate = "Mean estimate",
             coefString = "95% confidence interval") %>% 
  fmt_number(columns = c(estimate),
             rows = 1:10,
...
```{r, include=FALSE, eval=FALSE}
# extract predicted trends
pr <- ggeffects::predict_response(mod_moult_sex, c("date_J [30:293]", "sex"))
...
# plot the model
ggplot() +
  geom_line(data = mod_moult_sex_fits, 
            aes(x = date, y = predicted, color = sex)) +
  geom_ribbon(data = mod_moult_sex_fits, 
              aes(x = date, ymax = conf.high, ymin = conf.low, fill = sex),
              lwd = 1, alpha = 0.25) +
  geom_jitter(data = ind_prop_molt_scores,
              aes(x = date, y = prop_molt_score, color = sex), 
...
Yes: individuals that breed later than the rest of the population in a given year tend to commence their complete/post-breeding moult later
```{r}
...
Yes: individuals that breed later than the rest of the population in a given year tend to retain their full breeding plumage longer 
```{r}

...
#  read.csv("/Users/leberhart/ownCloud/kemp_projects/bdot/moult/data/Dataset_Molt_Final - modified 7.11.2024.csv", colClasses = "character") %>% 
  read.csv("data/Dataset_Molt_Final - modified 7.11.2024.csv", colClasses = "character") %>% 
...
#  read.csv("/Users/leberhart/ownCloud/kemp_projects/bdot/moult/data/sex_status_banding.csv", colClasses = "character") %>% 
  read.csv("data/sex_status_banding.csv", colClasses = "character") %>% 
  mutate(Rings_comb = str_sub(Rings_comb, 1, 4)) %>% 
  rename_with(tolower) %>% # convert all column headers to lowercase
  distinct()
...
### Seasonal changes in plumage score (week)

**Prepare the data**
Summerise the mean score and SD for each week for each sex separately

```{r}

dat_weekly_score <- dat %>% 
  mutate(
    week = ceiling(date_J / 7),
    score = as.numeric(score)
  ) %>% 
  filter(score != 0, !is.na(score)) %>%  # Remove 0 and NA scores
  filter(date_J >= 90 & date_J <= 302) %>%  # Keep only days between 1 Oct (90) and 30 Apr (302)
   group_by(sex, week) %>% 
  summarize(
  ...
  mutate(
    start_of_week = as.Date(paste0("2025-01-01")) + weeks(week - 27)
  )
...
ggplot(dat_weekly_score, aes(x = start_of_week, y = mean_score, fill = sex)) +
  geom_errorbar(aes(ymin = mean_score - sd_score, ymax = mean_score + sd_score), 
                width = 3, position = position_dodge(width = 2)) +  
  geom_point(shape = 21, size = 5, position = position_dodge(width = 2)) +
  theme(legend.position = "none",
        ...
        panel.grid.minor = element_blank()) +
  scale_x_date(date_labels = "%b %d", expand = c(0.01, 0.01), 
               date_breaks = "4 week", 
               limits = c(as.Date("2024-09-30"), as.Date("2025-05-01"))) + 
  ylab("Mean Score (mean ± SD)") +
  xlab("") + 
...
### Seasonal changes in plumage score (monthly)

**Prepare the data**
Summerise the mean score and SD for each month for each sex separately

```{r}

dat_monthly_score <- dat %>% 
  mutate(
    month = month(date),
    month = if_else(month <7, month + 6, month - 6),
    score = as.numeric(score)
  ) %>% 
  filter(score != 0, !is.na(score)) %>%  # Remove 0 and NA scores
  filter(date_J >= 90 & date_J <= 302) %>%  # Keep only days between 1 Oct (90) and 30 Apr (302)
  ...
  mutate(
    start_of_month = as.Date(paste0("2025-01-01")) + months(month - 7)
  )
...
#png("D:/OneDrive/Desktop/My Files/IRT/Dotterel molt - IRT3/R/plots/seasonal_score_change.png", width = 2000, height = 1400, res = 300)
ggplot(dat_monthly_score, aes(x = start_of_month, y = mean_score, fill = sex)) +
  geom_errorbar(aes(ymin = mean_score - sd_score, ymax = mean_score + sd_score), 
                width = 5, position = position_dodge(width = 10)) +  
  geom_point(shape = 21, size = 4.5, position = position_dodge(width = 10)) +
  theme(legend.position = "none",
        text = element_text(family = "Verdana"),
        axis.ticks = element_blank(),
        axis.text = element_text(size = 10, colour = "grey40"),
  ...
  scale_x_date(date_labels = "%B", expand = c(0.01, 0.01), 
               date_breaks = "1 month", 
               limits = c(as.Date("2024-09-15"), as.Date("2025-04-20"))) + # Adjust for your desired date range
  ylab("Rufous band score (mean ± SD)") +
  xlab("") + 
  scale_fill_brewer(palette = "Set2", direction = -1) +
  scale_y_continuous(limits = c(0, 7))
...
#png("D:/OneDrive/Desktop/My Files/IRT/Dotterel molt - IRT3/R/plots/moult_sex.png", width = 2400, height = 1600, res = 300)

grid.arrange(plot_rs_bird_sex, plot_rs_bird_fs_sex, ncol = 2)

#dev.off()

```

Get the start and end date of the moult
```{r}

# Extract coefficients for each ring_season
coefs <- coef(mod_moult_sex_bird_RIS)$ring_season

# Define start and end probability thresholds
t_start <- 0.975
t_end <- 0.025
logit_start <- qlogis(t_start)  # Logit transformation
logit_end <- qlogis(t_end)

# Calculate start and end dates for each ring_season
start_date_J <- (logit_start - coefs[, "(Intercept)"]) / coefs[, "date_J"]
end_date_J <- (logit_end - coefs[, "(Intercept)"]) / coefs[, "date_J"]

# Combine results into a data frame
moult_time <- data.frame(
  ring_season = rownames(coefs),
  start_date_J,
  end_date_J
) %>% 
  mutate(duration = end_date_J- start_date_J)

# Calculate mean and SD for start dates
mean_start <- mean(moult_time$start_date_J, na.rm = TRUE)
sd_start <- sd(moult_time$start_date_J, na.rm = TRUE)

# Calculate mean and SD for end dates
mean_end <- mean(moult_time$end_date_J, na.rm = TRUE)
sd_end <- sd(moult_time$end_date_J, na.rm = TRUE)


# Calculate mean and SD for duration
mean_duration <- mean(moult_time$duration, na.rm = TRUE)
sd_duration <- sd(moult_time$duration, na.rm = TRUE)

# Combine into a table
timing_mean_sd <- data.frame(
  Metric = c("Start Date", "End Date", "Duration"),
  Mean = c(mean_start, mean_end, mean_duration),
  SD = c(sd_start, sd_end, sd_duration)
)

print(timing_mean_sd)

# start: 176 --> 360 --> 26 Dec
# end: 247 --> 66 --> 7 March


#ggplot(mod_moult_sex_bird_RIS_fits, aes(x = date_J, y = predicted)) +
 # geom_point()+
  #geom_vline(xintercept = c(mean_start, mean_end))
...
#png("D:/OneDrive/Desktop/My Files/IRT/Dotterel molt - IRT3/R/plots/moult_status.png", width = 2400, height = 1600, res = 300)

grid.arrange(plot_rs_bird_status, plot_rs_bird_status_fs, ncol = 2)
...
#png("D:/OneDrive/Desktop/My Files/IRT/Dotterel molt - IRT3/R/plots/moult_age.png", width = 2400, height = 1600, res = 300)

grid.arrange(plot_rs_bird_age, plot_rs_bird_age_fs, ncol = 2)
...
fixed_effects <- fixef(mod_moult_age_bird_RIS)
random_effects <- ranef(mod_moult_age_bird_RIS)$ring_season

# Define start and end probability thresholds
t_start <- 0.975
t_end <- 0.025
logit_start <- qlogis(t_start)
logit_end <- qlogis(t_end)
...
# Loop through each ring_season and age
for (rs in rownames(random_effects)) {
  # Random effects for this ring_season
  b0_ring <- random_effects[rs, "(Intercept)"]  # Random intercept
  b_date_ring <- random_effects[rs, "date_J"]   # Random slope for date_J
  
  # Total slope (fixed + random)
  total_slope <- fixed_effects["date_J"] + b_date_ring
  
  # Loop through ages 1-6
  for (age_val in 1:6) {
    # Total intercept = fixed_intercept + random_intercept + (age_coef * age_value)
    total_intercept <- fixed_effects["(Intercept)"] + b0_ring + (fixed_effects["age"] * age_val)
    
    # Calculate dates
    start_date_J <- (logit_start - total_intercept) / total_slope
    end_date_J <- (logit_end - total_intercept) / total_slope
    duration <- end_date_J - start_date_J

    results_age <- rbind(results_age, data.frame(
      ring_season = rs,
      age = age_val,
      start_date_J,
      end_date_J,
...
summary_age <- results_age %>%
  group_by(age) %>%
  summarise(
    mean_start = mean(start_date_J, na.rm = TRUE),
    sd_start = sd(start_date_J, na.rm = TRUE),
    mean_end = mean(end_date_J, na.rm = TRUE),
    sd_end = sd(end_date_J, na.rm = TRUE),
...
ggplot(mod_moult_age_bird_RIS_fits, aes(x = date_J, y = predicted)) +
  geom_point()+
  geom_vline(data = summary_age, aes(xintercept = mean_start)) +
  geom_vline(data = summary_age, aes(xintercept = mean_end), linetype = "dashed")
...
plot_last_breeding_date_prop_molt_mid <- ggplot() +
  ...
  theme(text = element_text(family = "Verdana"),
        ...
        panel.spacing = unit(1, "cm"),
        plot.title = element_text(size = 14, hjust = -0.18),
        panel.background = element_rect(fill = 'transparent', color = NA),
        plot.background = element_rect(fill = 'transparent', color = NA), 
        panel.grid.major.y = element_line(size = 0.25, lineend = "round", colour = "grey90"),
        panel.grid.minor = element_blank(),
...
plot_late_initiation_date_prop_molt_mid <- ggplot() +
...
breeding_plumage_retention <- ggplot() +
...
png("D:/OneDrive/Desktop/My Files/IRT/Dotterel molt - IRT3/R/plots/moult_breeding.png", width = 4000, height = 1500, res = 300)

grid.arrange(plot_late_initiation_date_prop_molt_mid, plot_last_breeding_date_prop_molt_mid, breeding_plumage_retention, ncol = 3)
